...
ensure_local_file() {
  ...
  echo "[docker-run] created ${label} from example: ${target_path}"
}
...
if [[ ! -x "${BIN}" ]]; then
  ...
  echo "Build the image first with: ./scripts/docker_build.sh" >&2
  exit 1
fi
...
echo "[docker-run] adapter binary: ${BIN}"
echo "[docker-run] env file: ${ENV_FILE}"
echo "[docker-run] config path: ${CONFIG_PATH}"
